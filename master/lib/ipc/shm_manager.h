 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ── model_manager（进程 A，owner）───────────────────────────────────────────
 *
 *   agv::ShmOwner owner;
 *   owner.create_and_init();
 *   agv::ShmLayout* shm = owner.ptr();
 *
 *   // 写入初始地图（启动时，无需上锁，此时无其他进程）
 *   shm->map.node_count_ = 5;
 *   shm->map.nodes_[0] = { .id=0, .x=0, .y=0, .status=agv::NodeStatus::IDLE };
 *   // ...
 *
 *   // 进入主循环（阻塞于 mq_task_dispatch）
 *
 *
 * ── planner（消费者，频繁读地图）───────────────────────────────────────────
 *
 ...
 *   client.attach();
 *   agv::ShmLayout* shm = client.ptr();
 *
 *   // 每次规划时，先拿快照，再在本地副本上跑算法
 *   agv::MapData map_snap = agv::shm_read_map(shm);
 *   agv::CarData car_snap = agv::shm_read_cars(shm);
 *   // ... 用 map_snap / car_snap 做路径规划，完全不持有锁 ...
 *
 *
 * ── path_updater（ban/access 接口）─────────────────────────────────────────
 *
 *   // 找到对应 edge_idx，修改状态
 *   agv::shm_set_edge_status(shm, edge_idx, agv::EdgeStatus::BLOCKED);
 *
 *
 * ── mqtt_subscriber（到站事件）─────────────────────────────────────────────
 *
 *   agv::shm_set_car_status(shm, car_idx, agv::CarStatus::IDLE, arrived_node);
 *
 *
 * ── status API（HTTP GET /api/status）──────────────────────────────────────
 *
 *   agv::CarData snap = agv::shm_read_cars(shm);
 *   // 把 snap 序列化成 JSON 响应，不持有锁
 */